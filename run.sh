...
NUM_VIDEOS="${NUM_VIDEOS:-49}" # 38
HN_NUM_VIDEOS="${HN_NUM_VIDEOS:-18}" # 10
START_INDEX="${START_INDEX:-1}"
MAX_JOBS="${MAX_JOBS:-8}"
LOG_FILE="${LOG_FILE:-output5.log}"
...
for action_label in "${ACTION_LABELS[@]}"; do
  ...
  if [[ "$action_label" == hn_* ]]; then
    num_videos="$HN_NUM_VIDEOS"
  fi

  for ((i=START_INDEX; i<START_INDEX+num_videos; i++)); do
    echo "running ${action_label}_${i}" >> "$LOG_FILE"
